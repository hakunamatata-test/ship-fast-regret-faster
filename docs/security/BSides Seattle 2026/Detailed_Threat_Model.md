...
|---------|-------------|----------|
| SIEM Integration | Centralized security monitoring | All threats |
| Tool Usage Analytics | Detect anomalous tool patterns | E1, E2, E3 |
| Rate Limiting Alerts | Identify DoS attempts | D1, D2, D3 |
| Failed Auth Monitoring | Detect brute force/impersonation | S1, S3 |
| Data Exfiltration Detection | Identify unusual data access | I1, I2, I3 |
| Context Anomaly Detection | Detect poisoning attempts | T1, T3, T4 |

#### Preventive Controls

| Control | Description | Coverage |
|---------|-------------|----------|
| Input Validation | Block malicious inputs at ingress | T1, T4 |
| Authorization Gates | Enforce access policies | E1, E2, E3, E4 |
| Server Allowlisting | Only connect to trusted servers | S1, S2, S3 |
| Response Sanitization | Remove malicious content from outputs | T1, T3 |
| Rate Limiting | Prevent abuse and DoS | D1, D2, D3 |
| Network Segmentation | Isolate MCP components | All threats |

#### Corrective Controls

| Control | Description | Coverage |
|---------|-------------|----------|
| Incident Response Plan | Structured response to security events | All threats |
| Automated Rollback | Revert malicious changes | T1, T2, T3 |
| Server Blacklisting | Block compromised servers | S1, S3 |
| Session Termination | Kill compromised agent sessions | E4, T4 |
| Credential Rotation | Replace compromised credentials | S3, E3 |
...
- % of tools with least-privilege permissions
- % of high-risk operations with human approval
- Mean time to patch vulnerabilities

**Detective Metrics**:
- Number of malicious tool invocations detected per day
- % of prompt injection attempts blocked
- Mean time to detect security incident
- False positive rate on anomaly detection

**Corrective Metrics**:
- Mean time to respond to security incident
...
- Tool invocation rate by server
- Failed authorization attempts
- Anomaly detection alerts
- High-risk operation queue (pending approvals)

**Compliance Dashboard**:
- Audit log completeness (% of actions logged)
- Access control coverage (% of tools with authorization)
- Encryption coverage (% of connections using mTLS)
...
**Response Steps**:
1. **Isolate**: Immediately disconnect server from agent
2. **Assess**: Review audit logs for scope of compromise
3. **Contain**: Revoke server certificates, rotate credentials
4. **Eradicate**: Identify and remove malicious code
...
**Response Steps**:
1. **Terminate**: Kill affected agent session
2. **Analyze**: Extract and analyze poisoned context
3. **Trace**: Identify source (user input, tool response)
4. **Block**: Add pattern to injection detection rules
...
**Detection Indicators**:
- Mass data retrieval
- Access to unauthorized customer records
- Tool chaining to sensitive operations

**Response Steps**:
1. **Stop**: Terminate agent session, block actor
2. **Assess**: Determine scope of data accessed
3. **Notify**: Comply with breach notification requirements
4. **Investigate**: Review audit logs, identify root cause
5. **Remediate**: Tighten authorization policies
6. **Document**: Complete incident report for compliance
...
1. **MCP Creates New Attack Surfaces**: Every MCP server is a trust boundary; traditional perimeter security is insufficient.

2. **Context is a Weapon**: Shared agent context can be poisoned, manipulated, and exploited across server boundaries.

3. **Tool Chaining is Dangerous**: Low-privilege tools can be combined to achieve high-privilege outcomes.

4. **Identity is Critical**: Server impersonation and tool name collisions enable devastating attacks.

5. **Defense Requires Multiple Layers**: No single control is sufficient; defense-in-depth is essential.

6. **Observability is Security**: Comprehensive logging and anomaly detection are critical for detecting attacks.

### Critical Security Controls

**Must-Have Controls**:
1. Server authentication (mTLS, certificates)
2. Tool authorization (RBAC, ABAC)
...
8. Resource access controls
9. Anomaly detection
10. Incident response capabilities

### Design Principles

1. **Zero Trust**: Verify every server, every tool call, every response
2. **Least Privilege**: Minimize data exposure and permissions
3. **Defense in Depth**: Layer multiple controls
4. **Assume Breach**: Design with incident response in mind
5. **Visibility First**: You can't protect what you can't see
